...
export const OrderControl = () => {
    const totalState = useRecoilValue(totalOrderState);
        return (
        <form
            action={async () => await orderAction(totalState.orders)}
            className='flex flex-col justify-between fixed left-0 bottom-0 w-full h-[170px] bg-white p-[25px] rounded-t-2xl shadow-[0px_-4px_10px_0px_rgba(0,0,0,0.25)]'
        >
